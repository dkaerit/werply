...
const groups = [
  ...
  },
  {
    ...
    teams: [
      ...
      {
        label: "Luke Netteskjold",
        value: "Luke Netteskjold",
      },
    ],
  },
];
...
const filterFunction = (list: Team[], search: string) =>
  list.filter((i) => i.label? i.label.toLowerCase().includes(search.toLowerCase()): i);
  
const shouldShowSecondImage = computed(() => { // Check if the selected team belongs to group[1]
    ...
    <PopoverTrigger as-child>
      ...
      >
        ...
        <div id="bi-profile" class="flex -space-x-3 rtl:space-x-reverse pr-4">
         <img class="w-7 h-6 border-2 border-white rounded-full dark:border-gray-800" :src="`https://avatar.vercel.sh/${groups[0].teams[0].label}.png`" alt="">
         <img v-if="shouldShowSecondImage" class="w-6 h-6 border-2 border-white rounded-full dark:border-gray-800" :src="`https://avatar.vercel.sh/${selectedTeam.value}.png`" alt="">
        </div>
    ...
    <PopoverContent class="p-0 w-[237px]">
      <Command :filter-function="filterFunction">
        ...
        <CommandList>
          <CommandGroup v-for="group in groups" :key="group.label" :heading="group.label">
            ...
            >
              ...
              {{ team.label }}
              <Check :class="cn('ml-auto h-4 w-4', selectedTeam?.value === team.value ? 'opacity-100' : 'opacity-0')"/>
            </CommandItem>
          </CommandGroup>
        </CommandList>
        <CommandSeparator />
        ...
        <CommandList>
          ...
          <CommandList>
            <CommandGroup>
              <CommandItem id="logout" value="logout" class="cursor-pointer" @click="logout"> 
